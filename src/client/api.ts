...
export function createApi<T extends Api>(
  ...
  const on = <event extends Extract<keyof T['events'], string>>(
    event: event,
    cb: T['events'][event]
  ) => {
  ...
  const methods = (profile.methods || []).reduce(
    (acc, method) => ({
      ...acc,
...
export async function listenOnThemeChanged(
  client: PluginClient<any, any>,
  options?: Partial<PluginOptions<any>>
) {
  ...
  client.onload(async () => {
    client.on('theme', 'themeChanged', (_theme: Theme) =>
      setTheme(cssLink, _theme)
    );
