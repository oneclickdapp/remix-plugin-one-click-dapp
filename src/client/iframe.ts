...
export async function checkOrigin(
  ...
  const origins = devMode.origins
    ? typeof devMode.origins === 'string'
      ? [devMode.origins]
      : devMode.origins
    : [];
  const res = await fetch(
    'https://raw.githubusercontent.com/ethereum/remix-plugin/master/projects/client/assets/origins.json'
  );
  const defaultOrigins = await res.json();
  return [...defaultOrigins, ...localhost, ...origins].includes(origin);
}
...
export function buildIframeClient<T extends Api, App extends ApiMap = RemixApi>(
  ...
  // Add APIS
  const apis = getApiMap<ProfileMap<App>, App>(
    client,
    client.options.customApi
  );
