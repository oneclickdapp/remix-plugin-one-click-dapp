...
export class PluginClient<T extends Api = any, App extends ApiMap = RemixApi> {
  ...
  public call<
    Name extends Extract<keyof App, string>,
    Key extends MethodKey<App[Name]>
  >(
    name: Name,
    key: Key,
  ...
  public on<
    Name extends Extract<keyof App, string>,
    Key extends EventKey<App[Name]>
  >(name: Name, key: Key, cb: EventCallback<App[Name], Key>): void {
  ...
  public emit<Key extends EventKey<T>>(
    key: Key,
