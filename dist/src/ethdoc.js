...
export function createDoc(result) {
    return Object
    ...
    const methodsDoc = contract.abi.map((def) => {
        if (def.type === 'constructor') {
            ...
            const methodDoc = {
                ...
                notice: contract.userdoc.methods.constructor
            };
...
const getParams = (params) => (params.length === 0)
    ? '_No parameters_'
...
function getMethodDoc(def, devdoc) {
    const doc = devdoc || {};
